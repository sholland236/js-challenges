...
export const findType = () => {
  ...
  let typeOfThing;
  switch (type) {
    case "string": typeOfThing = "This is a string"; break;
    case "number": typeOfThing = "This is a number"; break;
    case "boolean": typeOfThing = "This is a number"; break;
    default: typeOfThing = "I don't know what this thing is";
  }
  return typeOfThing;
};
...
export const getIsValidOnNameTag = () => {
  ...
  if (nameTagOption.length <= 8 && isUpperCase(nameTagOption.charAt(0))) {
    return true;
  } else {
    return false;
  }
...
export const convertStringToNumber = () => {
  let number = parseFloat(stringToConvert);
  return number;
};
...
export const getHasUppercaseLetters = () => {
  // create the regexp to use, eg any capital letters: [A-Z]
  let reg = /[A-Z]/;
  // use test with item to test as argument, against regexp created
  return reg.test(stringWithUppercaseLetters);
};
...
export const convertPascalCaseToSnakeCase = () => {
  // split() will split the code at the given input, eg split(" ") will split a sentence each time there is a space
  // regexp input in this case: ?= will 'look ahead' (positive) for any capital letter
  // there for the input will split each time a capital letter is encountered
  const splitWords = pascalCaseVariableName.split(/(?=[A-Z])/);
  // then join the separated words with _
  const joinedWordsWithUnderscores = splitWords.join("_");
  // whole thing to lower case
